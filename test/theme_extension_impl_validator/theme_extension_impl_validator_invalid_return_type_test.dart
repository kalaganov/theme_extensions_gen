...
void main() {
  test('fails when return type is not ThemeExtension or List<ThemeExtension>',
      () async {
          ...
          throwsA(
            predicate(
              (e) =>
                  ...
                  e.message.contains(
                    'Return type must be ThemeExtension '
                    'or List<ThemeExtension>.',
                  ),
            ),
          ),
        );
...
@ThemeExtensionImpl()
Iterable<ThemeExtension> themeExtensionsFunction() {
  return [
    $implSource,
  ];
}
